...
class EventService(
    ...
    fun createEvent(request: EventRequest, userId: Long): EventResponse {
        ...
        val event = Event(user = user, title = request.title).apply {
    ...
    fun getEvents(): List<EventResponse> {
        return eventRepository.findAll().map(EventResponse::fromEntity)
    }
    ...
    fun addTagToEvent(eventId: Long, request: EventTagRequest): EventTagResponse {
        val event = eventRepository.findById(eventId).orElseThrow {
            NotFoundException("Event not found")
        }
    ...
    fun removeTagFromEvent(eventId: Long, tagId: Long) {
        val event = eventRepository.findById(eventId).orElseThrow {
            NotFoundException("Event not found")
        }
        val tag = tagRepository.findById(tagId).orElseThrow {
    ...
    private fun parseEventStatus(status: String): EventStatus {
        return EventStatus.entries.firstOrNull { it.name.equals(status, ignoreCase = true) }
